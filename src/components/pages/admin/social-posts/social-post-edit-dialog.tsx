...
import { useAdminControllerUpdateSocialPost } from '@/lib/api/generated/admin/admin';
...
type UpdateSocialPostFormData = z.infer<typeof updateSocialPostSchema>;

export function SocialPostEditDialog({
  socialPost,
  open,
  onOpenChange,
  onClose,
  onRefetch,
}: SocialPostEditDialogProps) {
  ...
  const form = useForm<UpdateSocialPostFormData>({
    resolver: zodResolver(updateSocialPostSchema),
    defaultValues: {
      platform: socialPost.platform,
      postUrl: socialPost.postUrl,
      imageUrl: String(socialPost.imageUrl || ''),
      videoUrl: String(socialPost.videoUrl || ''),
      description: String(socialPost.description || ''),
      publisher: String(socialPost.publisher || ''),
  ...
  const updateSocialPostMutation = useAdminControllerUpdateSocialPost({
    mutation: {
      onSuccess: () => {
      ...
      onError: (error: unknown) => {
        toast.error(
          (error as { response?: { data?: { message?: string } } })?.response
            ?.data?.message || 'Failed to update social post'
        );
  ...
  const onSubmit = async (data: UpdateSocialPostFormData) => {
    ...
    try {
      ...
      // Update social post
      await updateSocialPostMutation.mutateAsync({
        id: socialPost.id,
        data: {
  ...
  const currentImageUrl = imagePreview || String(socialPost.imageUrl || '');
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    ...
                    <p>{new Date(socialPost.createdAt).toLocaleDateString()}</p>
                  </div>
                  <div>
                    <p className="text-muted-foreground">Last Updated</p>
                    <p>{new Date(socialPost.updatedAt).toLocaleDateString()}</p>
                  </div>
                </div>
              </div>
            </div>
